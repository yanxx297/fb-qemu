...
KEMUFUZZER_HYPERCALL_START_TESTCASE = 0x1e
KEMUFUZZER_HYPERCALL_STOP_TESTCASE  = 0x1f
...
def dump_page_table(cr3, mem):
    ...
    for i in range(1024):
        pde = read_dword((cr3 & ~0xfff) + i * 4)
        if pde & 0x1:
            r += "  PDE[%.4d]: %.8x\n" % (i, pde & ~0xfff)
            for j in range(1024):
                pte = read_dword((pde & ~0xfff) + j * 4)
                if pte & 0x1:
                    r += "    PTE[%.4d]: %.8x" % (j, pte & ~0xfff)
                    r += "\t(%.8x --> %.8x)\n" % (i << 22 | j << 12, 
                                                  pte & ~0xfff)
...
class X86DumpCpuState(cpu_state_t):
    def __str__(self):
        ...
        # page table
        # s += "\n* Page table\n"
        # s += dump_page_table(self.sregs_state.cr3, dump.mem.data)
...
class X86Dump():
    ...
    def parse_idt_desc(self, i, idx):
        ...
        # Read GDT entry
        idt_base = cpu.sregs_state.idtr.base
        idt_limit = cpu.sregs_state.idtr.limit

        # Extract i-th entry
        assert idx*8 <= idt_limit
        addr = idx*8 + idt_base
        data = self.mem.data[addr:addr+8]

        idtdesc = idt_desc_t()

        tmp = data[0] + data[1] + data[6]+ data[7]
        idtdesc.offset = struct.unpack("I", tmp)[0]
        idtdesc.selector = unpack8(data[2]) | (unpack8(data[3]) << 8)
        idtdesc.type = unpack8(data[5]) & 0x1f
        idtdesc.dpl = (unpack8(data[5]) >> 5) & 0x3
        idtdesc.d = (unpack8(data[5]) >> 3) & 0x1
        idtdesc.present = (unpack8(data[5]) >> 7) & 0x1
    ...
    def guess_guest_ctx(self, i):
        ...
        # ===---------------------------------------------------------------===
        # This is not needed anymore since we hook software interrupts and
        # exceptions
        # ===---------------------------------------------------------------===
        # rsp = cpu.regs_state.rsp + cpu.sregs_state.ss.base
    ...
    def __str__(self):
        ...
        for i in range(self.hdr.cpusno):
            ...
            s += "\n[GDT #%d]\n" % i
            for sel in range(0,51):
                seg = self.parse_seg(i, sel << 3)
                s += "   [%.2d] [%.2x] Base:%.8x Limit:%.8x Type:%.4x \n" % (sel, seg.selector, seg.base, seg.limit, seg.type)
                
            s += "\n[IDT #%d]\n" % i
            for idx in range(self.cpus[i].sregs_state.gdtr.limit / 8):
                idtdesc = self.parse_idt_desc(i, idx)
                s += "   [%.2d] [%.2x] Selector:%.3x Offset:%.8x Type:%.8x Present:%.1x DPL:%.1x D:%.1x\n" % (idx, idx, idtdesc.selector, idtdesc.offset, idtdesc.type, idtdesc.present, idtdesc.dpl, idtdesc.d)
...
def parse_header(buf):
    return my_from_buffer_copy(X86DumpHeader, buf)
